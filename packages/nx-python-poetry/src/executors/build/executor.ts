...
/**
 * Builds the current project and all if it's local dependencies into a single wheel/package. This
 * is done by creating a temporary build folder, copying the project and it's dependencies to the
 * folder, and then building them all in the same folder.
 */
export default async function executor(
  ...
  try {
    ...
    const buildFolderPath = path.join('dist', root);
    ...
    // Resolve the dependencies and write them to the build pyproject.toml
    ...
    // Copy the built artifacts to the final build folder and do some cleanup
